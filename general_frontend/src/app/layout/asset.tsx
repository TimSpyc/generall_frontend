...
const Asset = (props: AssetProps): JSX.Element => {
  ...
  if (
    props.children.filter(
      (child: ReactElement) =>
        child.props.type === (props.defaultView ? props.defaultView : "default")
    ).length <= 0
  ) {
    throw new Error(
      `KnowledgeHub: The view ${props.name} needs exact one default page or change the defaultView Property on your asset to have an existing default page assigned`
    );
  }
    ...
    <>
      ...
      >
        {props.children.map((child: JSX.Element, index: number) => {
          if (
            child.props.hasOwnProperty("type") === false ||
            child.props.type === view
          ) {
            return (
          ...
          {props.buttons.map((data: any, index: number) => {
            if (data.view.includes(view)) {
              return (
                <button
                  key={index}
                  className="py-0.5 px-1 bg-black text-white text-xs mb-2 rounded-md"
                  onClick={(event) =>
                    handleActions(data.action, data.actionProps, event)
                  }
                >
                  {data.label}
                </button>
