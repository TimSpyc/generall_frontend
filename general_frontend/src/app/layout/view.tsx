...
const View = (props: ViewProps): JSX.Element => {
  ...
  const processedChildren = () => {
    ...
    React.Children.map(props.children, (child: JSX.Element, index: number) => {
      let element =
        childrenSize[view][child.props.name][
          `${parentLayout.w}x${parentLayout.h}`
        ];
      ...
      return (
        ...
        >
          <div className="relative w-full h-full p-1">
            {isViewDraggable && (
              <div
                onClick={() => removeChild(child.props.name)}
                className="absolute top-1.5 right-1.5 w-4 h-4 bg-gray-600 rounded-full text-xs shadow-md text-white leading-[initial] cursor-pointer border border-white flex items-center justify-center"
              >
                <span>x</span>
              </div>
            )}
            {cloneElement(child, {
              key: child.props.name,
              tabIndex: `${props.index}000${
                sortedIndizes.findIndex(
                  (element: any) => element.i === child.props.name
                ) + 1
              }`,
              gridSize:
                childrenSize[view][child.props.name][
  ...
  const FetchRequest = async (key: string, value: any) => {
    ...
    let params = value.params
      ? `?${new URLSearchParams(value.params).toString()}`
      : "";

    return (data[key] = useSWR(
      `${url}${value.id ? value.id : ""}${params}`,
      fetcher,
      swrConfig
    ));
  };
  ...
  const removeChild = (idx: string) => {
    setChildrenSize((currentState: any) => {
      currentState[view][idx][`${parentLayout.w}x${parentLayout.h}`][
        "visible"
      ] = false;
    ...
    >
      {isViewDraggable && (
        <>
          ...
          >
            <div className="p-2 py-3 w-full shadow-md">
              <p className="text-black">
                Hidden Items {currentLayout[assetName].w}x
                {currentLayout[assetName].h}
              </p>
            </div>
            ...
            {childrenSize[view] && (
              <div className="w-full h-full overflow-y-auto">
                {Object.entries(childrenSize[view]).map(([idx, view]: any) => {
                  if (
                    view[`${parentLayout.w}x${parentLayout.h}`].visible ===
                    false
                  ) {
                    return (
                      <div
                        onClick={() => addChild(idx)}
                        className="text-black w-full p-2 px-3 first-of-type:border-t border-b border-black cursor-crosshair"
                        key={idx}
                      >
                        {idx}
                      </div>
          ...
          <div
            className={`absolute z-40 bottom-4 left-4 bg-gray-800 text-white px-1 py-0.5 rounded-md shadow-md`}
          >
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className={`transition-all ease-in-out flex flex-row gap-2 text-xs`}
            >
              <div
                className={`${
              ...
              {
                processedChildren().filter(
                  (child: any) =>
                    childrenSize[view][child.key][
                      `${parentLayout.w}x${parentLayout.h}`
                    ].visible === false
                ).length
              }{" "}
